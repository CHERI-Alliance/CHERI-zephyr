...
int main(void)
{
	printk("\nRunning CHERI k_heap overflow sample on %s\n", CONFIG_BOARD_TARGET);

	/* Pointer to memory allocated */
	void *ptr1;

	/* Allocate memory */
	ptr1 = k_heap_alloc(&kernel_heap, ALLOC1, K_NO_WAIT);

	if (ptr1 == 0) {
		printk("k_heap_alloc failed!\n");
		return -1;
	}

	printk("k_heap allocation address: %p\n", ptr1);
	print_cheri("", ptr1);
	printk("\nmax index: %zu\n", (size_t)INDEX_SZ);
	...
	printk("k_heap allocation overflowed!\n");
	printk("Sample finished\n");
