			  ...
			  unsigned int index_limit)
{
	...
	if (index >= index_limit) {
		...
		/*
		 * To print unsigned long we need to cast from pointer
		 * type to unsigned long which are different sizes in CHERI
		 */
		LOG_ERR("inappropriate PMP range (start=%#lx size=%#zx)", (unsigned long)start,
			size);
#else
