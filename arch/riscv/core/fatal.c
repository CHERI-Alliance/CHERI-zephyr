...
const char *z_riscv_cap95_cause_str(unsigned long mtval2)
{
	...
	static const char *const cap_cause_str[16] = {
		[0] = "CHERI tag violation",
		[1] = "CHERI seal violation",
		[2] = "CHERI permission violation",
		[3] = "CHERI invalid address violation",
		[4] = "CHERI bounds violation",
		[5 ... 15] = "CHERI reserved",
	};
	return cap_cause_str[MIN(exccode, ARRAY_SIZE(cap_cause_str) - 1)];
}
...
#else
		#ifdef __CHERI_PURE_CAPABILITY__
		LOG_ERR("     cs0: " PR_REG "    cs6: " PR_REG, (unsigned long)csf->cs0, (unsigned long)csf->cs6);
		LOG_ERR("     cs1: " PR_REG "    cs7: " PR_REG, (unsigned long)csf->cs1, (unsigned long)csf->cs7);
		LOG_ERR("     cs2: " PR_REG "    cs8: " PR_REG, (unsigned long)csf->cs2, (unsigned long)csf->cs8);
		LOG_ERR("     cs3: " PR_REG "    cs9: " PR_REG, (unsigned long)csf->cs3, (unsigned long)csf->cs9);
		LOG_ERR("     cs4: " PR_REG "   cs10: " PR_REG, (unsigned long)csf->cs4, (unsigned long)csf->cs10);
		LOG_ERR("     cs5: " PR_REG "   cs11: " PR_REG, (unsigned long)csf->cs5, (unsigned long)csf->cs11);
		#else
