	...
	if(CONFIG_CHERI)
		if(CONFIG_64BIT)
			...
			message(STATUS, "Compiling for riscv32 CHERI-PURECAP new RISC-V spec v0.9.5")
				...
				#For cores that boot in hybrid mode we need to build for zcherihybrid (-march),
				#whilst keeping the ABI for purecap (-mabi)
				string(PREPEND CMAKE_ASM_FLAGS "-march=rv32imafd_zcherihybrid -mabi=il32pc64 ")
				string(PREPEND CMAKE_C_FLAGS   "-march=rv32imafd_zcherihybrid -mabi=il32pc64 ")
				string(PREPEND CMAKE_CXX_FLAGS "-march=rv32imafd_zcherihybrid -mabi=il32pc64 ")
				else()
		...
		string(PREPEND CMAKE_ASM_FLAGS "-march=rv64imafc -mabi=lp64")
		string(PREPEND CMAKE_C_FLAGS   "-march=rv64imafc -mabi=lp64")
		string(PREPEND CMAKE_CXX_FLAGS "-march=rv64imafc -mabi=lp64")
		else()
...
#-------------------------------------------------
  ...
  elseif("${triple}" STREQUAL "riscv64" OR "${triple}" STREQUAL "riscv64-unknown-elf")
	#get only the name of the directory, then, create the full path
	if(CONFIG_CHERI)
		set(LIBGCC_DIR ${TOOLCHAIN_HOME}../baremetal/baremetal-riscv64-purecap/lib/)
	else()
