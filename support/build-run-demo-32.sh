...
DEMO_DIR=/home/user/zephyrproject/demo-32
TOOL_DIR=/opt/cheri
...
west build -p always -b qemu_riscv32cheri samples/subsys/modbus/rtu_server_cheri
...
west build -p always -b qemu_riscv32cheri samples/subsys/modbus/rtu_client_cheri
...
tmux new-session -d 'sleep 1;'"$TOOL_DIR"'/qemu-codasip-patched/build/qemu-system-riscv32cheri -nographic -machine virt -bios none -m 256 -net none -pidfile qemu.pid -chardev stdio,id=con,mux=on -serial chardev:con -mon chardev=con,mode=readline -serial pipe:null-cable-1 -icount shift=6,align=off,sleep=on -rtc clock=vm -kernel client.elf' \; split-window -h 'sleep 1.5;'"$TOOL_DIR"'/qemu-codasip-patched/build/qemu-system-riscv32cheri -nographic -machine virt -bios none -m 256 -net none -pidfile qemu2.pid -chardev stdio,id=con,mux=on -serial chardev:con -mon chardev=con,mode=readline -serial pipe:null-cable-2 -icount shift=6,align=off,sleep=on -rtc clock=vm -kernel server.elf' \; attach-session
