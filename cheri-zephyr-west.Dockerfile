...
# Import LLVM from cheri-llvm stage
# Copy without setting permissions always results 
# in root ownership, COPY always succeeds because 
# the directory exists in Stage 1 - even if empty
# Always copy, if not included will copy empty dir.
# Matches with last 4 lines in cheri-llvm.Dockerfile
COPY --from=cheri-llvm /home/user/cheri/output $TOOL_DIR/cheri/output
COPY --from=cheri-llvm /home/user/qemu-codasip/build $TOOL_DIR/qemu-codasip/build
COPY --from=cheri-llvm /home/user/qemu-codasip-patched/build $TOOL_DIR/qemu-codasip-patched/build
COPY --from=cheri-llvm /home/user/llvm-cheri-codasip/build $TOOL_DIR/llvm-cheri-codasip/build
COPY --from=cheri-llvm /home/user/gdb-codasip/gdb $TOOL_DIR/gdb-codasip/gdb
COPY --from=cheri-llvm /home/user/llvm-builtins-codasip $TOOL_DIR/llvm-builtins-codasip
