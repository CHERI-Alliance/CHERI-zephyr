...
# Running docker CHERI-Zephyr-v0.1.0
...
```
cd zephyrproject
docker build --no-cache -t zephyrimage -f docker-release/CHERI-Zephyr-v0.1.0/zephyrdockerfile .
```
### To build wih buildx:
```
docker buildx build -t zephyrimage -f docker-release/CHERI-Zephyr-v0.1.0/cheridockerfile --load .
```
### To run
```
...
```
cd zephyrproject
docker build --no-cache -t cherizephyrimage -f docker-release/CHERI-Zephyr-v0.1.0/cheridockerfile .
```
### To select features:
```
docker build --no-cache --build-arg FEATURE_CHERIBUILD=false --build-arg FEATURE_CODASIP=true -t cherizephyrimage -f docker-release/CHERI-Zephyr-v0.1.0/cheridockerfile .
```
### To build wih buildx:
```
docker buildx build -t cherizephyrimage -f docker-release/CHERI-Zephyr-v0.1.0/cheridockerfile --load .
docker buildx build --build-arg FEATURE_CHERIBUILD=false --build-arg FEATURE_CODASIP=true -t cherizephyrimage -f docker-release/CHERI-Zephyr-v0.1.0/cheridockerfile --load .
```
### To run
```
