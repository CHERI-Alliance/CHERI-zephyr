 ...
 */
 ...
 /* We have used a separate file for the CHERI modified functions
  * because there were a lot of changes regarding the use of a
  * structure to accommodate capability types instead of unsigned long
  * types which is defined in an array.
  *
  * A small change to CMakeLists selects this file using CONFIG_CHERI
  * when building for CHERI without impacting the original test script.
  *
  * Note that this script is also valid for non-cheri builds. For example
  * replacing both CONFIG_CHERI and __CHERI_PURE_CAPABILITY__ with
  * CONFIG_TOOLCHAIN_LLVM_CHERI would run this code in both modes.
  *
  * Note for cheri builds this test relies on memcpy being cheri ported
  * to allow capabilities to be copied without losing tags.
  */
...
/* The test uses unsigned long type for a value and a pointer.
 * For cheri a pointer type is bigger so we need to set
 * this up differently. Here we use a structure.
 */
struct msgq_msg {
	union {
		unsigned long value;
...
K_MSGQ_DEFINE(manager_q, sizeof(struct msgq_msg), 4, 4);
#else
...
#ifdef __CHERI_PURE_CAPABILITY__
static ZTEST_DMEM struct msgq_msg service1_buf[1];
static ZTEST_DMEM struct msgq_msg service2_buf[1];
static ZTEST_DMEM struct msgq_msg client_buf[2];
#else
...
#ifdef __CHERI_PURE_CAPABILITY__
/* This version of service_manager_entry uses the struct method
 * accomodating a capability type which is necessary to make this
 * cheri compatible.
 */
static void service_manager_entry(void *p1, void *p2, void *p3)
{
	struct msgq_msg data;

	while (1) {
		k_msgq_get(&manager_q, &data, K_FOREVER);
		switch (data.field[0].value) {
		case QUERRY_SERVICE:
			pclient = data.field[1].queue;
			k_msgq_put(pclient, services, K_NO_WAIT);
			break;
		case REGISTER_SERVICE1:
			services[0] = data.field[1].queue;
			k_sem_give(&service_started);
			break;
		case REGISTER_SERVICE2:
			services[1] = data.field[1].queue;
			k_sem_give(&service_started);
			break;
		case SERVICE_QUIT:
			for (int i = 0; i < NUM_SERVICES; i++) {
				if (services[i] == data.field[1].queue) {
					services[i] = NULL;
				}
			}
			/* wake up threads waiting for this queue */
			k_msgq_purge(data.field[1].queue);
			break;
		default:
			TC_PRINT("Unknown message %lu\n", data.field[0].value);
			break;
		}
		k_msleep(10);
	}
...
#ifdef __CHERI_PURE_CAPABILITY__
/* This version of service1_entry uses the struct method
 * accomodating a capability type which is necessary to make this
 * cheri compatible.
 */
static void service1_entry(void *p1, void *p2, void *p3)
{
	struct msgq_msg service_data;
	struct k_msgq *client;
	int ret;

	service_data.field[0].value = REGISTER_SERVICE1;
	service_data.field[1].queue = &service1_msgq;

	k_msgq_init(&service1_msgq, (char *)service1_buf,
			sizeof(struct msgq_msg), 1);
	...
	while (service1_run) {
		...
		if (service_data.field[0].value == GET_SERVICE) {
			client = service_data.field[1].queue;

			service_data.field[0].value = SERVICE1_RUNNING;
	...
	service_data.field[0].value = SERVICE_QUIT;
	service_data.field[1].queue = &service1_msgq;
...
#ifdef __CHERI_PURE_CAPABILITY__
/* This version of service2_entry uses the struct method
 * accomodating a capability type which is necessary to make this
 * cheri compatible.
 */
static void service2_entry(void *p1, void *p2, void *p3)
{
	...
	service_data.field[0].value = REGISTER_SERVICE2;
	service_data.field[1].queue = &service2_msgq;
	...
	while (service2_run) {
		...
		if (service_data.field[0].value == GET_SERVICE) {
			...
			service_data.field[0].value = SERVICE2_RUNNING;
			k_msgq_put(client, &service_data, K_NO_WAIT);
		}
		k_msleep(10);
	}
...
#ifdef __CHERI_PURE_CAPABILITY__
/* This version of client_entry uses the struct method
 * accomodating a capability type which is necessary to make this
 * cheri compatible.
 */
static void client_entry(void *p1, void *p2, void *p3)
{
	struct msgq_msg client_data;
	struct msgq_msg service_data;
	...
	k_msgq_init(&client_msgq, (char *)client_buf,
		    sizeof(struct msgq_msg), 2);

	client_data.field[0].value = QUERRY_SERVICE;
	client_data.field[1].queue = &client_msgq;
	...
	service1q = service_data.field[0].queue;
	service2q = service_data.field[1].queue;
	...
	while (1) {
		...
		client_data.field[0].value = GET_SERVICE;
		client_data.field[1].queue = &client_msgq;
		...
		if (service1q) {
			...
			if (!ret) {
				zassert_equal(service_data.field[0].value,
