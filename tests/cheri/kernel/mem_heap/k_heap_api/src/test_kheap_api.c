...
static void thread_alloc_heap(void *p1, void *p2, void *p3)
{
	...
	/* Extra CHERI checks */
	TC_PRINT("k_heap_test.heap.heap: %p\n", k_heap_test.heap.heap);
	print_cheri("k_heap_test\n:", k_heap_test.heap.heap);
	TC_PRINT("ALLOC_SIZE_2: %zu\n", (size_t)ALLOC_SIZE_2);
	TC_PRINT("p: %p\n", p);
	print_cheri("p\n:", p);
	assert_cheri_invalid_tag("p", p);
	/* End extra CHERI checks */
 ...
 * its memory bounds.
 */
ZTEST(k_heap_api_cheri, test_k_heap_min_size)
{
	...
	/* Extra CHERI checks */
	TC_PRINT("tiny_heap: %p\n", tiny_heap.heap.heap);
	print_cheri("tiny_heap:\n", tiny_heap.heap.heap);
	TC_PRINT("p0: %p\n", p0);
	print_cheri("p0:\n", p0);
	TC_PRINT("p1: %p\n", p1);
	print_cheri("p1:\n", p1);
	assert_cheri("tiny_heap", tiny_heap.heap.heap, Z_HEAP_MIN_SIZE);
	assert_cheri("p0", p0, sizeof(void *));
	assert_cheri_invalid_tag("p1", p1);
	/* End extra CHERI checks */
 ...
 */
ZTEST(k_heap_api_cheri, test_k_heap_alloc)
{
	...
	TC_PRINT("k_heap_test.heap.heap: %p\n", k_heap_test.heap.heap);
	print_cheri("k_heap_test\n:", k_heap_test.heap.heap);
	TC_PRINT("ALLOC_SIZE_1: %zu\n", (size_t)ALLOC_SIZE_1);
	TC_PRINT("p: %p\n", p);
	print_cheri("p\n:", p);
	assert_cheri("p", p, ALLOC_SIZE_1);
	/* End extra CHERI checks */
 ...
 * @see k_heap_malloc(), k_heap_Free()
 */
ZTEST(k_heap_api_cheri, test_k_heap_alloc_fail)
{
 ...
 */
ZTEST(k_heap_api_cheri, test_k_heap_free)
{
	k_timeout_t timeout = Z_TIMEOUT_US(TIMEOUT);
	char *p = (char *)k_heap_alloc(&k_heap_test, ALLOC_SIZE_1, timeout);
 ...
 * @ingroup cheri_k_heap_api_tests
 */
ZTEST(k_heap_api_cheri, test_kheap_alloc_in_isr_nowait)
{
 ...
 * @ingroup cheri_k_heap_api_tests
 */
ZTEST(k_heap_api_cheri, test_k_heap_alloc_pending)
{
	/*
 ...
 * @ingroup cheri_k_heap_api_tests
 */
ZTEST(k_heap_api_cheri, test_k_heap_alloc_pending_null)
{
	/*
	 * Allocate first to make sure subsequent allocations
	 * either fail (K_NO_WAIT) or pend.
 ...
 */
ZTEST(k_heap_api_cheri, test_k_heap_calloc)
{
	k_timeout_t timeout = Z_TIMEOUT_US(TIMEOUT);
	...
	TC_PRINT("k_heap_test.heap.heap: %p\n", k_heap_test.heap.heap);
	print_cheri("k_heap_test\n:", k_heap_test.heap.heap);
	TC_PRINT("CALLOC_SIZE: %zu\n", (size_t)CALLOC_SIZE);
	TC_PRINT("CALLOC_NUM: %zu\n", (size_t)CALLOC_NUM);
	TC_PRINT("p: %p\n", p);
	print_cheri("p:\n", p);
	assert_cheri("p", p, CALLOC_SIZE * CALLOC_NUM);
	/* End extra CHERI checks */
